...
- [x] Port each engine class — done 2026-07-17 under `d365/FileWatcherMonitoring.Plugins/`, with a vector-driven xunit suite in `d365/FileWatcherMonitoring.Plugins.Tests/` (**not yet compiled** — no .NET SDK in this sandbox; first `dotnet test` runs in the client env)
- [ ] Tick each matrix row ⬜→✅ once `dotnet test` is green in the client environment (keep the matrix the single source of progress)
